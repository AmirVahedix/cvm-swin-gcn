...
def set_stored_instance_id(
    instance_id: int, env_path: Optional[Path] = None
) -> None:
    """Store active instance ID in .env."""
    update_env_variable("INSTANCE_ID", str(instance_id), env_path)
    update_env_variable("VAST_INSTANCE_ID", str(instance_id), env_path)


def clear_stored_instance_id(env_path: Optional[Path] = None) -> None:
    """Clear INSTANCE_ID and VAST_INSTANCE_ID from .env."""
    update_env_variable("INSTANCE_ID", "", env_path)
    update_env_variable("VAST_INSTANCE_ID", "", env_path)
    update_env_variable("instance_id", "", env_path)
...
def generate_tmux_runner_script(
    ...
    """Generate bash script to run setup.sh inside a tmux session and handle auto-destruction."""
    pipeline_args_str = " ".join(
        f"'{a}'" if (" " in a or "$" in a) else a for a in pipeline_args
    )
    iid_str = str(instance_id) if instance_id else ""
    auto_destroy_str = "true" if auto_destroy else "false"

    return f"""#!/bin/bash
# ==============================================================================
# Vast.ai Detached Tmux Runner & Lifecycle Watchdog
# Automatically generated by vast_runner.py
# ==============================================================================

INSTANCE_ID="{iid_str}"
VAST_API_KEY="{api_key}"
AUTO_DESTROY="{auto_destroy_str}"
GRACE_PERIOD_SEC={grace_period_sec}
PIPELINE_ARGS=({pipeline_args_str})

LOG_FILE="/workspace/pipeline.log"
STATUS_FILE="/workspace/pipeline.status"
EXIT_CODE_FILE="/workspace/pipeline.exitcode"
PID_FILE="/workspace/pipeline.pid"

# Record startup
echo $$ > "$PID_FILE"
echo "RUNNING" > "$STATUS_FILE"
rm -f "$EXIT_CODE_FILE" /workspace/pipeline.done

echo "=================================================================" | tee "$LOG_FILE"
echo "🚀 Starting Vast.ai Remote Pipeline in tmux session 'train'" | tee -a "$LOG_FILE"
echo "⏰ Start Time: $(date)" | tee -a "$LOG_FILE"
echo "📦 Instance ID: ${{INSTANCE_ID:-N/A}}" | tee -a "$LOG_FILE"
echo "⚙️ Auto-destroy on completion: ${{AUTO_DESTROY}}" | tee -a "$LOG_FILE"
echo "=================================================================" | tee -a "$LOG_FILE"

# Execute setup.sh with unbuffered output
export PYTHONUNBUFFERED=1
bash /workspace/setup.sh -y --run-pipeline "${{PIPELINE_ARGS[@]}}" 2>&1 | tee -a "$LOG_FILE"
EXIT_CODE=${{PIPESTATUS[0]}}

echo "$EXIT_CODE" > "$EXIT_CODE_FILE"
echo "FINISHED" > "$STATUS_FILE"
touch /workspace/pipeline.done
echo "⏰ Finished Time: $(date)" | tee -a "$LOG_FILE"

if [ $EXIT_CODE -eq 0 ]; then
    echo "" | tee -a "$LOG_FILE"
    echo "=================================================================" | tee -a "$LOG_FILE"
    echo "🎉 PIPELINE FINISHED SUCCESSFULLY (Exit Code: 0)" | tee -a "$LOG_FILE"
    echo "✅ Model checkpoints, artifacts, and evaluation logged to MLflow." | tee -a "$LOG_FILE"
    echo "=================================================================" | tee -a "$LOG_FILE"

    if [ "$AUTO_DESTROY" = "true" ] && [ -n "$INSTANCE_ID" ] && [ -n "$VAST_API_KEY" ]; then
        echo "⏳ Waiting ${{GRACE_PERIOD_SEC}}s grace period for local artifact download / log sync..." | tee -a "$LOG_FILE"
        sleep "$GRACE_PERIOD_SEC"
        echo "🗑️ Auto-destroying Vast.ai instance ${{INSTANCE_ID}} via API to stop billing..." | tee -a "$LOG_FILE"
        curl -s -k -X DELETE "https://console.vast.ai/api/v0/instances/${{INSTANCE_ID}}/?api_key=${{VAST_API_KEY}}" | tee -a "$LOG_FILE"
        echo "" | tee -a "$LOG_FILE"
        echo "✅ Destroy signal sent. Instance terminating." | tee -a "$LOG_FILE"
    else
        echo "ℹ️ Auto-destroy not active or missing credentials. Instance ${{INSTANCE_ID}} kept alive." | tee -a "$LOG_FILE"
    fi
else
    echo "" | tee -a "$LOG_FILE"
    echo "=================================================================" | tee -a "$LOG_FILE"
    ...
    if [ "$AUTO_DESTROY" = "true" ] && [ -n "$INSTANCE_ID" ] && [ -n "$VAST_API_KEY" ]; then
        FAIL_GRACE=$((GRACE_PERIOD_SEC * 2))
        echo "⏳ Waiting ${{FAIL_GRACE}}s before auto-destroying failed instance..." | tee -a "$LOG_FILE"
        sleep "$FAIL_GRACE"
        echo "🗑️ Auto-destroying failed Vast.ai instance ${{INSTANCE_ID}} via API..." | tee -a "$LOG_FILE"
        curl -s -k -X DELETE "https://console.vast.ai/api/v0/instances/${{INSTANCE_ID}}/?api_key=${{VAST_API_KEY}}" | tee -a "$LOG_FILE"
    fi
fi
"""
...
def run_remote_pipeline_tmux(
    ...
    """Launch setup & pipeline inside a detached tmux session on the remote instance and optionally stream logs."""
    extra_pipeline_args = extra_pipeline_args or []
    runner_script = generate_tmux_runner_script(
    ...
    print(
        f"\n{COLOR_BLUE}🚀 Deploying detached tmux runner to remote instance...{COLOR_RESET}"
    )

    # Deploy runner script remotely and ensure tmux is installed
    setup_cmd = (
        "mkdir -p /workspace && "
        "cat << 'EOF_TMUX_RUNNER' > /workspace/run_tmux_pipeline.sh\n"
        f"{runner_script}\n"
        "EOF_TMUX_RUNNER\n"
        "chmod +x /workspace/run_tmux_pipeline.sh && "
        "which tmux >/dev/null 2>&1 || (apt-get update -qq && apt-get install -y tmux -qq) && "
        "tmux kill-session -t train 2>/dev/null || true && "
        "rm -f /workspace/pipeline.done /workspace/pipeline.status /workspace/pipeline.exitcode && "
        "tmux new-session -d -s train 'bash /workspace/run_tmux_pipeline.sh'"
    )
    ...
    deploy_res = subprocess.run(ssh_deploy_cmd)
    if deploy_res.returncode != 0:
        raise RuntimeError("Failed to launch tmux session on the remote instance.")

    print(
        f"{COLOR_GREEN}✅ Training pipeline launched inside remote tmux session 'train'!{COLOR_RESET}"
    )
    print(
        f"{COLOR_CYAN}ℹ️ Safe to close your laptop! Training will continue running on the GPU and will auto-destroy when finished.{COLOR_RESET}"
    )

    if detach:
        print(f"\n{COLOR_BOLD}🔗 Detached mode active. Monitor your job with:{COLOR_RESET}")
        print(f"   python3 scripts/vast_runner.py attach {instance_id or ''}".rstrip())
        print(f"   python3 scripts/vast_runner.py logs -f {instance_id or ''}".rstrip())
        return 0

    print(
        f"\n{COLOR_BLUE}📡 Streaming live logs from remote tmux session (press Ctrl+C to detach):{COLOR_RESET}\n"
        + "=" * 70
        + "\n"
    )
    ...
    tail_cmd = [
        ...
        str(ssh_port),
        f"{user}@{ssh_host}",
        "touch /workspace/pipeline.log && tail -n +1 -f /workspace/pipeline.log",
    ]

    tail_proc = subprocess.Popen(tail_cmd)

    try:
        while tail_proc.poll() is None:
            # Check if finished
            check_cmd = [
                "ssh",
                "-o",
                ...
                str(ssh_port),
                f"{user}@{ssh_host}",
                "[ -f /workspace/pipeline.done ] && echo 'DONE' || echo 'RUNNING'",
            ]
            ...
            if check_res.returncode == 0 and "DONE" in check_res.stdout:
                time.sleep(3)
                tail_proc.terminate()
                break
            time.sleep(4)
        ...
        tail_proc.terminate()
        print(
            f"\n\n{COLOR_YELLOW}⚠️ Disconnected from live log stream (laptop closed / Ctrl+C).{COLOR_RESET}"
        )
        print(
            f"{COLOR_GREEN}✅ The remote training is STILL running safely in tmux on the GPU!{COLOR_RESET}"
        )
        print(f"{COLOR_CYAN}You can reconnect anytime using:{COLOR_RESET}")
        print(f"   python3 scripts/vast_runner.py attach {instance_id or ''}".rstrip())
        print(f"   python3 scripts/vast_runner.py logs -f {instance_id or ''}".rstrip())
        print(
            f"{COLOR_DIM}When finished, the instance will automatically upload artifacts and self-destruct.{COLOR_RESET}\n"
        )
        raise
    ...
    exitcode_cmd = [
        ...
        str(ssh_port),
        f"{user}@{ssh_host}",
        "cat /workspace/pipeline.exitcode 2>/dev/null || echo '1'",
    ]
    ...
    raw_code = code_res.stdout.strip()
    try:
        return int(raw_code)
    except ValueError:
        return 0 if "DONE" in code_res.stdout else 1
...
def run_remote_pipeline(
    ...
    """Execute setup.sh with --run-pipeline directly on the instance with live output streaming."""
    extra_pipeline_args = extra_pipeline_args or []
    pipeline_args_str = " ".join(extra_pipeline_args)
    cmd_str = f"bash {remote_dest}/setup.sh -y --run-pipeline {pipeline_args_str}"

    print(f"\n{COLOR_BLUE}🚀 Executing remote setup & pipeline directly:{COLOR_RESET}")
...
def cmd_run(args: argparse.Namespace, client: VastAPIClient) -> None:
    """Complete end-to-end automation: search/create or attach manually, setup, train in tmux, and manage lifecycle."""
    instance_id = None
    created_new = False
    has_error = False
    error_reason = ""
    ...
    try:
        manual_mode = getattr(args, "manual", False) or (
            ssh_host is not None and ssh_port is not None
        )
        ...
        if manual_mode:
            ...
            if not ssh_host:
                ssh_host = input(
                    f"{COLOR_CYAN}Enter SSH Host (IP / Domain, e.g., 74.50.x.x or ssh4.vast.ai): {COLOR_RESET}"
                ).strip()
            if not ssh_host:
            ...
            if not ssh_port:
                raw_port = input(
                    f"{COLOR_CYAN}Enter SSH Port (e.g., 12345): {COLOR_RESET}"
                ).strip()
        ...
        # Execute training with tmux decoupling
        auto_destroy = getattr(args, "destroy_on_finish", True) and not getattr(
            args, "stop_on_finish", False
        )
        use_tmux = not getattr(args, "no_tmux", False)

        if use_tmux:
            ret_code = run_remote_pipeline_tmux(
                ssh_host=ssh_host,
                ssh_port=ssh_port,
                instance_id=instance_id,
        ...
        if instance_id:
            if getattr(args, "detach", False):
                pass
            elif args.destroy_on_finish and not has_error:
                print(
                    f"\n{COLOR_YELLOW}🗑️ Auto-destroying instance {instance_id} as requested (--destroy-on-finish)...{COLOR_RESET}"
                )
                try:
                    client.destroy_instance(instance_id)
                    clear_stored_instance_id()
                    print(f"{COLOR_GREEN}✅ Instance {instance_id} destroyed.{COLOR_RESET}")
                except Exception as e:
                    print(
                        f"{COLOR_RED}❌ Failed to destroy instance (might already be destroyed by remote watchdog): {e}{COLOR_RESET}"
                    )
                    clear_stored_instance_id()
                ...
                try:
                    ...
                    print(f"{COLOR_GREEN}✅ Instance {instance_id} stopped.{COLOR_RESET}")
                except Exception as e:
                    print(
                ...
                if should_destroy:
                    ...
                    print("   python3 scripts/vast_runner.py attach")
                    print("   python3 scripts/vast_runner.py logs -f")
                    print("   python3 scripts/vast_runner.py destroy")
            else:
                print(
                    f"\n{COLOR_CYAN}ℹ️ Instance {instance_id} is running in background. Manage it when done:{COLOR_RESET}"
                )
                print("   python3 scripts/vast_runner.py attach    # view tmux")
                print("   python3 scripts/vast_runner.py logs -f   # tail logs")
...
def cmd_stop(args: argparse.Namespace, client: VastAPIClient) -> None:
    ...
    if not instance_id:
        print(f"{COLOR_RED}❌ No instance ID specified!{COLOR_RESET}")
        sys.exit(1)
    client.stop_instance(instance_id)
    print(
...
def cmd_destroy(args: argparse.Namespace, client: VastAPIClient) -> None:
    ...
    clear_stored_instance_id()
    print(
        f"{COLOR_GREEN}✅ Destroy command issued for instance {instance_id}. Stored ID cleared.{COLOR_RESET}"
    )


def cmd_attach(args: argparse.Namespace, client: VastAPIClient) -> None:
    """Attach interactively to the remote tmux session."""
    instance_id = get_stored_instance_id(args.instance_id)
    if not instance_id:
        print(
    ...
    session_name = getattr(args, "session", "train") or "train"
    print(
        f"\n{COLOR_BLUE}📺 Attaching to remote tmux session '{session_name}' on instance {instance_id} ({ssh_host}:{ssh_port})...{COLOR_RESET}"
    )
    print(f"{COLOR_DIM}Detach anytime with: Ctrl+b then d{COLOR_RESET}\n")
    ...
    ssh_cmd = [
        ...
        str(ssh_port),
        f"root@{ssh_host}",
        f"tmux attach-session -t {session_name} || tmux attach || tmux new-session -s {session_name}",
    ]
    subprocess.run(ssh_cmd)


def cmd_logs(args: argparse.Namespace, client: VastAPIClient) -> None:
    """View or follow the remote pipeline log file."""
    instance_id = get_stored_instance_id(args.instance_id)
    if not instance_id:
        print(
            f"{COLOR_RED}❌ No instance ID specified and none found in .env!{COLOR_RESET}"
    ...
    follow_flag = "-f" if getattr(args, "follow", False) else ""
    lines_flag = f"-n {args.lines}" if getattr(args, "lines", None) else "-n 50"

    print(
        f"\n{COLOR_BLUE}📜 Fetching pipeline logs from instance {instance_id} ({ssh_host}:{ssh_port})...{COLOR_RESET}\n"
    )
    ...
    ssh_cmd = [
        ...
        str(ssh_port),
        f"root@{ssh_host}",
        f"tail {follow_flag} {lines_flag} /workspace/pipeline.log",
    ]
    try:
        subprocess.run(ssh_cmd)
    except KeyboardInterrupt:
        print(f"\n{COLOR_YELLOW}Stopped log streaming.{COLOR_RESET}")
...
def cmd_ssh(args: argparse.Namespace, client: VastAPIClient) -> None:
    ...
    if getattr(args, "tmux", False):
        ssh_cmd.insert(1, "-t")
        ssh_cmd.append(
            "tmux attach-session -t train || tmux attach || tmux new-session -s train"
        )
    elif args.command:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_p.add_argument(
        "--stop-on-finish",
        action="store_true",
        help="Automatically stop the instance when pipeline finishes successfully (pauses billing)",
    )
    run_p.add_argument(
        "--destroy-on-finish",
        dest="destroy_on_finish",
        action="store_true",
        default=True,
        help="Automatically destroy the instance when pipeline finishes successfully (default: True)",
    )
    run_p.add_argument(
        "--no-destroy",
        "--keep-alive",
        dest="destroy_on_finish",
        action="store_false",
        help="Keep the instance alive after pipeline completes instead of auto-destroying",
    )
    run_p.add_argument(
        "--no-tmux",
        "--direct",
        action="store_true",
        help="Run pipeline directly in foreground without tmux",
    )
    run_p.add_argument(
        "-d",
        "--detach",
        action="store_true",
        help="Launch pipeline in background tmux session and exit CLI immediately",
    )
    run_p.add_argument(
        "--timeout-destroy",
        type=int,
    ...
    attach_p = subparsers.add_parser(
        "attach", help="Attach interactively to the remote tmux session"
    )
    attach_p.add_argument(
        ...
        nargs="?",
        default=None,
        help="Instance ID to attach to (defaults to INSTANCE_ID in .env)",
    )
    attach_p.add_argument(
        "--session",
        type=str,
        default="train",
        help="tmux session name (default: 'train')",
    )

    # 5. Logs
    logs_p = subparsers.add_parser(
        "logs", help="View or stream remote pipeline logs"
    )
    logs_p.add_argument(
        "instance_id",
        type=int,
        nargs="?",
        default=None,
        help="Instance ID to fetch logs from (defaults to INSTANCE_ID in .env)",
    )
    logs_p.add_argument(
        "-f",
        "--follow",
        action="store_true",
        help="Follow log stream in real time",
    )
    logs_p.add_argument(
        "-n",
        "--lines",
    ...
    ssh_p.add_argument(
        "--tmux",
        action="store_true",
        help="Attach directly to tmux session on the instance",
    )
...
def main():
    ...
    if args.subcommand == "search":
        ...
        cmd_attach(args, client)
    elif args.subcommand == "logs":
        cmd_logs(args, client)
